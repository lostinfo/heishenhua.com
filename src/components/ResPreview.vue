<template>
  <div class="gs-res-preview">
    ...
    >
      <div v-if="showData[showActive].items[showActive2].video" class="play-button">
      </div>
      <div class="show-control" :class="showFullScreen?'full-screen':''">
        <div
          class="show-ctl-left"
          v-if="showActive2 > 0"
  ...
  methods: {
    ...
    showCtlLeft: function() {
      ...
      if (this.showFullScreen) {
        ...
        this.currentVideo = this.showData[this.showActive].items[this.showActive2].video;
      }
    },
    ...
    scrollJudge: function() {
      ...
      // }
      this.thumbnailScrollTo(this.showActive2 * this.itemWith - this.thumbnailClientWidth / 2 + this.itemWith / 2);
    },
    ...
    thumbnailScrollToSmooth: function(startScrollLeft, offset, idx, max, interval) {
      if (idx < max) {
        this.$refs.showThumbnail.scrollLeft = startScrollLeft + offset * (idx + 1);
        setTimeout(
          () => {
            this.thumbnailScrollToSmooth(startScrollLeft, offset, idx + 1, max, interval);
          },
          interval
        );
      }
