...
function scrollRegister(el) {
  scrollElement = el;
  let mouseEvent = "onwheel" in document.createElement("div") ? "wheel" :
...
function onMouseWheel(deltaY) {
  ...
  if ((scrollOffset >= 0) && (deltaY < 0)) {
    ...
    setScroll(scrollElement.scrollTop, scrollStart + scrollOffset - scrollElement.scrollTop + deltaY);
  }
  startScroll();
}

function setScroll(start, offset, stime=scrollDefaultTime, scale=scrollDefaultScale) {
  scrollStart = start;
  scrollOffset = offset;
  scrollTime = stime;
  scrollTimeCurrent = 0;
